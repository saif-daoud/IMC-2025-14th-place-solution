...
def read_csv(filename, target_datasets=None, header=True, print_header=False):
    data = {}
    label_idx = {}    
...
def score(
    *,
    ...
    gt_data = read_csv(gt_csv, target_datasets)
    ...
    for dataset in tqdm(gt_data.keys(), total=len(gt_data)):
        ...
        for i, gt_scene in enumerate(gt_dataset.keys()):                                  
            ...
            if verbose:
                print(f'scene {gt_scene}: mAA={best_mAA[i] * 100:.2f}%')
